...
// https://stackoverflow.com/questions/2904877/user-agent-parsing-using-c-sharp
// https://github.com/ua-parser/uap-csharp/tree/master/UAParser
// https://github.com/faisalman/ua-parser-js/blob/master/src/ua-parser.js
// http://www.c-sharpcorner.com/blogs/uapraser-user-agent-parser1
namespace UAParser
{
    ...
    public sealed class Parser
    {
        ...
        Parser(MinimalYamlParser yamlParser)
        {
            ...
            Device defaultDevice = new Device(other, "", "");
        ...
        static IEnumerable<T> Read<T>(IEnumerable<Dictionary<string, string>> entries, Func<Func<string, string>, T> selector)
        {
            ...
            foreach (Dictionary<string, string> cm in entries)
            {
                ls.Add(selector(cm.Find));
            }
        ...
        {
            System.Reflection.Assembly asm = typeof(Parser).Assembly;

            string yaml = null;
            foreach (string str in asm.GetManifestResourceNames())
            {
                if (str.EndsWith("UAParser.regexes.yaml", System.StringComparison.OrdinalIgnoreCase))
                {
                    yaml = str;
                    break;
                } // End if(str.EndsWith("UAParser.regexes.yaml", System.StringComparison.OrdinalIgnoreCase)) 

            } // Next str 


            using (System.IO.Stream strm = asm.GetManifestResourceStream(yaml))
            {
        ...
        static Func<string, TResult> CreateParser<T, TResult>(IEnumerable<Func<string, T>> parsers, T defaultValue, Func<T, TResult> selector) where T : class
        {
            ...
            System.Collections.Generic.List<Func<string, T>> lsParsers = new List<Func<string, T>>();

            if (parsers != null)
            {
                foreach (Func<string, T> thisParser in parsers)
                {
                    lsParsers.Add(thisParser);
                }
            }

            return delegate(string ua)
            {
                T parsedValue = defaultValue;

                foreach (Func<string, T> p in lsParsers)
                {
                    T parseValue = p(ua);
                    if (parseValue != null)
                    {
